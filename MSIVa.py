...
df_ms4a = df_ms.loc[df_ms.MetaSanitaria == 'MSIVa']
df_ms4a = df_ms4a.merge(df_est, on='IdEstablecimiento', how='left')
#%%
# Preparar los datos
df_ms4a = df_ms4a.dropna(subset=['Ano', 'Mes'])
df_ms4a['Ano'] = df_ms4a['Ano'].astype(int)
df_ms4a['Mes'] = df_ms4a['Mes'].astype(int)
df_ms4a['Porcentaje'] = df_ms4a['Numerador']/df_ms4a['Denominador']
df_ms4a['IdEstablecimiento'] = df_ms4a['IdEstablecimiento'].astype(str)
df_ms4a['nombre_establecimiento'] = df_ms4a['nombre_establecimiento'].astype(str)
df_ms4a = df_ms4a.dropna(subset=["servicio_salud", "comuna"])
df_ms4a["servicio_salud"] = df_ms4a["servicio_salud"].fillna("No especificado").astype(str)
df_ms4a["comuna"] = df_ms4a["comuna"].fillna("No especificado").astype(str)
df_ms4a['codigo_nombre']=df_ms4a['IdEstablecimiento']+' - '+df_ms4a['nombre_establecimiento']
#%%
...
# -----------------------------------------------------
all_servicios = ["Todos"] + sorted(df_ms4a["servicio_salud"].unique())
...
df_ms4a_filtered = (
    ...
    else df_ms4a[df_ms4a["servicio_salud"] == selected_servicios]
)
...
all_comunas = sorted(df_ms4a_filtered["comuna"].unique())
...
if "Todas" not in selected_comunas:
    df_ms4a_filtered = df_ms4a_filtered[df_ms4a_filtered["comuna"].isin(selected_comunas)]


# -----------------------------------------------------
# 5) FILTRO 3: Dependencia Administrativa
# -----------------------------------------------------
all_dependencias = sorted(df_ms4a_filtered["Dependencia Administrativa"].dropna().unique())
...
if "Todas" not in selected_dependencias:
    df_ms4a_filtered = df_ms4a_filtered[df_ms4a_filtered["Dependencia Administrativa"].isin(selected_dependencias)]


# -----------------------------------------------------
# 6) FILTRO 4: Nivel de Atención
# -----------------------------------------------------
all_niveles = sorted(df_ms4a_filtered["Nivel de Atención"].dropna().unique())
...
if "Todos" not in selected_niveles:
    df_ms4a_filtered = df_ms4a_filtered[df_ms4a_filtered["Nivel de Atención"].isin(selected_niveles)]


# -----------------------------------------------------
# 7) FILTRO 5: Establecimientos
# -----------------------------------------------------
all_establecimientos = sorted(df_ms4a_filtered["codigo_nombre"].dropna().unique())
...
if "Todos" not in selected_establecimientos:
    df_ms4a_filtered = df_ms4a_filtered[df_ms4a_filtered["codigo_nombre"].isin(selected_establecimientos)]


# -----------------------------------------------------
# 8) FILTRO de Mes (ejemplo)
# -----------------------------------------------------
all_meses = sorted(df_ms4a_filtered['Mes'].unique())
selected_meses = st.selectbox('Seleccione mes de corte', all_meses, index=len(all_meses)-1)
df_ms4a_filtered = df_ms4a_filtered[df_ms4a_filtered['Mes'] == selected_meses]
...
num_services = df_ms4a_filtered['servicio_salud'].nunique()
num_communes = df_ms4a_filtered['comuna'].nunique()
num_establishments = df_ms4a_filtered['codigo_nombre'].nunique()
...
#%%
# Mostrar dataframe
col_ms4a=['Ano', 'Mes','codigo_nombre','servicio_salud',  'Numerador', 'Denominador','Porcentaje']
rename_ms4a={
    'Ano':'Año', 
    'Mes':'Mes',
...
df_cumplimiento = df_ms4a_filtered.groupby('comuna').agg(
